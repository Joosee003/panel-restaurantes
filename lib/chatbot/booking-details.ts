...
export function explicitParty(text: string) {
...
export function bookingIntentDetails(text: string) {
  ...
  const time = parseRequestedTime(text, service);
...
export function parseRequestedTime(text: string, preferredService?: MealService) {
  ...
  const match = value.match(/\b(?:a las?|sobre las?|hacia las?|para las?|hora(?: es| seria)?[: ]*)\s+(\d{1,2})(?:(?:[:.]|\s+)([0-5]\d)|\s+(y media|y cuarto|menos cuarto))?(?:\s*(am|pm|h|horas?))?(?=\s|[.!?]?$)/)
    ...
    || value.match(/^(\d{1,2})(?:(?:[:.]|\s+)([0-5]\d)|\s+(y media|y cuarto|menos cuarto))?(?:\s*(am|pm|h|horas?))?(?=\s|[.!?]?$)/);
