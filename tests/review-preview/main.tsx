...
function Preview(){
 ...
 return <main className="mx-auto min-h-screen max-w-6xl p-4 sm:p-7">
  <header className="mb-5 flex flex-wrap items-start justify-between gap-3"><div><p className="text-sm font-semibold text-indigo-600">GASTROHELP · VISTA DE PRUEBA</p><h1 className="mt-1 text-3xl font-black">Reseñas</h1><p className="mt-2 max-w-2xl text-sm text-slate-500">Datos ficticios. La petición de WhatsApp se simula automáticamente al abrir esta prueba.</p></div><div className="flex flex-wrap gap-2"><a className={b} href="./mobile.html">Ver ancho de móvil</a><button className={b} onClick={()=>setDark(v=>!v)}>{dark?'Modo claro':'Modo oscuro'}</button></div></header>
