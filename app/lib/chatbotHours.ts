...
export type ChatbotServiceRange = { service: string; start: string; end: string };
type Range = ChatbotServiceRange;
...
async function dayRanges(supabase: Pick<SupabaseClient, "from">, restaurantId: string, date: string, rows: ScheduleRow[]): Promise<Range[] | null> {
...
export async function readChatbotServiceRanges(supabase: Pick<SupabaseClient, "from">, restaurantId: string, date: string) {
  const { data, error } = await supabase.from("reservas_horarios")
    .select("dia_semana,turno,hora_inicio,hora_fin,activo").eq("restaurante_id", restaurantId);
  if (error) throw new Error("CHATBOT_HOURS_UNAVAILABLE");
  return dayRanges(supabase, restaurantId, date, (data || []) as ScheduleRow[]);
}
...
export async function readChatbotHours(
  ...
  if (date) {
    const ranges = await dayRanges(supabase, restaurantId, date, rows);
