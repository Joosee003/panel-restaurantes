...
export default function ResenasPage() {
  ...
  return <div className={`min-h-screen space-y-6 px-4 py-6 sm:px-6 lg:px-8 ${dark ? "bg-slate-950 text-white" : "bg-slate-50 text-slate-950"}`}>
    <header><div className="flex items-center gap-3"><span className="rounded-2xl bg-indigo-100 p-3 text-indigo-700"><MessageCircle size={24}/></span><div><h1 className="text-3xl font-black tracking-tight">Reseñas</h1><p className={`mt-1 text-sm ${dark ? "text-slate-400" : "text-slate-500"}`}>Pide su opinión después de la visita y comprueba las reseñas en Google.</p></div></div></header>
