...
export type ReviewPanelClient = {
  ...
  reviewLink?: (token: string) => string;
  preparedMessage?: (message: { token: string; text: string; phone: string }) => void;
  inspectGoogle?: () => void;
};
...
export default function ReviewRequestsPanelView({ restauranteId, dark, client }: { restauranteId: string; dark: boolean; client: ReviewPanelClient }) {
  ...
  const [confirmation, setConfirmation] = useState<{ request: ReviewRequest; action: string; text: string } | null>(null);
  const confirmationDialog = useRef<HTMLDialogElement>(null);
  useEffect(() => {
    if (confirmation && !confirmationDialog.current?.open) confirmationDialog.current?.showModal();
    if (!confirmation && confirmationDialog.current?.open) confirmationDialog.current.close();
  }, [confirmation]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const configured = await client.channelConfigured(restauranteId);
        if (active) setChannelConfigured(configured);
  ...
  const action = async (request: ReviewRequest, name: string) => {
    const { data: result, error: actionError } = await client.rpc("visit_review_action", { p_reserva_id: request.reserva_id, p_action: name });
    if (actionError) throw new Error(actionError.message);
    return result;
  };

  const runAction = async (request: ReviewRequest, name: string, accepted = false) => {
    if (busy) return;
    ...
    if (prompts[name] && !accepted) { setConfirmation({ request, action: name, text: prompts[name] }); return; }
  ...
  const prepare = async (request: ReviewRequest, copy: boolean) => {
    ...
    const popup = copy || client.preparedMessage ? null : window.open("about:blank", "_blank");
    ...
    try {
      ...
      const link = client.reviewLink?.(prepared.token) || `${window.location.origin}/r/${prepared.token}`;
      const message = reviewMessage(prepared.name, prepared.restaurantName, link);
      if (client.preparedMessage) {
        client.preparedMessage({ token: prepared.token, text: message, phone });
        setNotice("Mensaje preparado. El envío sigue pendiente de confirmar.");
      } else if (copy) {
  ...
  const save = async () => {
    ...
    const { error: saveError } = await client.rpc("save_visit_review_settings", {
  ...
  return <div className="space-y-5">
    <dialog ref={confirmationDialog} onCancel={() => setConfirmation(null)} aria-labelledby="review-confirm-title" aria-describedby="review-confirm-description" className={`m-auto w-[calc(100%_-_2rem)] max-w-md rounded-2xl border p-6 shadow-xl backdrop:bg-slate-950/50 ${dark ? "border-slate-700 bg-slate-900 text-white" : "border-slate-200 bg-white text-slate-900"}`}>
      <h2 id="review-confirm-title" className="text-lg font-bold">Comprobar antes de guardar</h2>
      <p id="review-confirm-description" className={`mt-3 text-sm leading-6 ${muted}`}>{confirmation?.text}</p>
      <div className="mt-5 flex flex-wrap justify-end gap-2"><button type="button" className={button} onClick={() => setConfirmation(null)}>Cancelar</button><button type="button" className={primary} onClick={() => { if (confirmation) void runAction(confirmation.request, confirmation.action, true); }}>Sí, lo he comprobado</button></div>
    </dialog>
          ...
          {requests.map(request => {
            ...
            return <article key={request.reserva_id} className={`rounded-2xl border p-4 ${dark ? "border-slate-700 bg-slate-950" : "border-slate-200"}`}>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                {googleReviewUrl(data?.settings.google_review_url) && <a href={googleReviewUrl(data?.settings.google_review_url)!} target="_blank" rel="noopener noreferrer" onClick={client.inspectGoogle ? event => { event.preventDefault(); client.inspectGoogle?.(); } : undefined} className={button}><ExternalLink size={16}/> Revisar Google</a>}
