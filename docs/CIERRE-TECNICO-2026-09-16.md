...
|---|---|---|---|---|---|
| Alta eliminada al fallar el correo | API anterior ejecutaba rollback tras error o resultado incierto | Alta | Instalación persistente, clave UUID, ledger y bloqueo SQL; invitación independiente. SQL y API probadas | Publicado; pruebas locales y visuales superadas | Correo y sesión agencia reales pendientes |
| Pérdida de avance al recargar | Wizard anterior solo useState | Alta | Borrador por usuario y pestaña, 24 h, clave estable; borrado al cambiar sesión; prueba de desmontaje/recarga y segunda cuenta | Publicado; recuperación en vista previa verificada | Navegador privado real pendiente |
| Invitación presentada como enviada sin evidencia | pending/sent compartían descripción | Alta | Estados no solicitada, enviando, aceptada por proveedor, fallida e incierta. Reintento explícito con bloqueo y espera. No se equipara aceptación a entrega | Publicado; lógica y vista previa verificadas | Recepción real pendiente |
| Reenvío a cuenta creada | Cuenta podía existir tras fallo parcial | Alta | Recuperación de acceso solo a usuario vinculado y correo comprobado; página admite invite/recovery y rechaza enlace ausente, caducado o tipo indebido | Publicado; lógica local verificada | Auth/SMTP real |
| Retorno al número común para reseñas | Dispatcher aceptaba null del canal propio y seguía ruta compartida | Alta | Bloquea con own_whatsapp_number_required; prueba de cero envíos | Publicado; pruebas locales superadas | Número propio y comprobación real |
| Canal compartido contado como preparado | Indicadores de alta permitían sharedConnected | Media | Chatbot y reseñas requieren canal propio; se conserva información de la ruta anterior | Publicado; pruebas locales superadas | Número propio y comprobación real |
| Datos personales en demo | API pública permitía reservas en webs es_demo; demo visible contiene entradas de origen no acreditado | Alta | Bloqueo 403 en API y formulario sustituido por aviso de solo lectura; cero escrituras en prueba. No se han borrado registros existentes | API publicada: 403 DEMO_READ_ONLY verificado; revisión de datos pendiente | Clasificar entradas de demo con el titular |
...
- PR 69 integrada: commit `11b348fbe5ab19f1a95b0300a4e8badbff4167b9`, despliegue `dpl_AYkaVeYWSGdhn8wUdmvLbJEXyG3R`, READY, asignado a `panel.gastrohelp.es`. La migración es compatible con el código anterior.
- Vista previa final `62fb455f8643d4dfed5a241529d4b8bedd3bd662`, `dpl_9BDbE3dmVKvV4ELWZ1UD9mx1Sapv`, READY y comprobación Vercel correcta. Modo oscuro revisado: indicador positivo `#6ee7b7`, contraste 11,02:1 sobre `#151d30`, sin desbordamiento.
- Comprobación HTTP de producción: login 200; resumen de agencia y reenvío de invitación sin sesión 401; reserva de demo 403 `DEMO_READ_ONLY`; fixture visual ausente en producción (404). Evidencia: `docs/evidence/2026-09-16/production-release-checks.json`. La petición de demo usa fecha pasada y ningún contacto: también el código anterior la rechazaría antes de escribir.
- Navegador en producción: acceso demo, dashboard con cinco tarjetas alineadas y ancho 1348/1348; consulta de disponibilidad pública y bloqueo del formulario de datos. La revisión detectó dos textos antiguos que prometían guardar reservas; se corrigen para explicar el modo de solo lectura desde el inicio. No se cambia la marca de La Reserva ni sus enlaces.
